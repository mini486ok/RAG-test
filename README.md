# 🚄 Rail-Brain Test Lab — 철도·교통 특화 AI 개발 테스트

**Basic LLM vs Vector RAG vs Graph RAG**를 한 화면에서 실시간으로 비교하는 철도·교통 특화 AI 개발 테스트 웹앱입니다.
...
| **문서 업로드 → 자동 DB 구축** | PDF · DOCX · PPTX · HWPX · MD · TXT 지원, 브라우저에서 직접 파싱. **증분 구축**(새 문서만 기존 DB에 누적) 기본, 전체 재구축 별도 제공 |
| **Vector DB** | 문장 인식 청킹 → bge-m3 임베딩 → 코사인 Top-K + MMR 재순위화 |
| **Graph DB** | LLM 기반 개체·관계 추출(추출 전용 소형 모델 선택 가능) → 지식그래프 구축 → 시드 개체 탐색 + n-hop 확장 |
| **컨텍스트 수용성 게이지** | 질의마다 RAG 증강 프롬프트가 num_ctx 예산의 몇 %인지 표시 (사전 추정 → 생성 후 실측 갱신, 초과분 자동 절삭 경고) |
